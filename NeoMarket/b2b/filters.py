...
class B2CProductFilter(django_filters.FilterSet):
    category_id = django_filters.UUIDFilter(field_name='category')
    seller_id = django_filters.UUIDFilter(field_name='seller')
    ...
    class Meta:
        model = Product
        fields = ['category_id', 'seller_id', 'search', 'ids', 'min_price', 'max_price']
    ...
    def filter_queryset(self, queryset):
        # Сначала применяем все стандартные фильтры
        for name, value in self.form.cleaned_data.items():
            if value is not None:
                queryset = self.filters[name].filter(queryset, value)
        
        # Затем обрабатываем динамические фильтры по характеристикам filters[...]
        filters_data = {}
        for param in self.request.GET:
            if param.startswith('filters[') and param.endswith(']'):
                key = param[8:-1]  # Извлекаем имя характеристики
                filters_data[key] = self.request.GET.getlist(param)
        ...
        for char_name, char_values in filters_data.items():
            if not char_values:
                continue
            # char_values - это список значений (уже плоский от getlist)
